...
const GameUpload = () => {
  ...
  useEffect(() => {
    const fetchTeachersAndAdmins = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/teachers-and-admins');
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      const gamePath = `/games/${file.name}`; // Generate the game path from the file name
      setGamePath(gamePath);  // Store the generated game path

      // Set the game URL dynamically before uploading
      setGameUrl(`http://localhost:3000${gamePath}`);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/upload-game', formData, {
      ...
      setGameUrl(`http://localhost:3000${response.data.gamePath}`); // Set the game URL dynamically
      setUploading(false);
    } catch (error) {
      setUploading(false);
